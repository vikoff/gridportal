...
class VomsController extends Controller{
	...
	public function display_view($params = array()){
		...
		$instanceId = getVar($params[0], 0, 'int');
			...
			$variables = Voms::Load($instanceId)->GetAllFieldsPrepared();
			FrontendViewer::get()
				->setTitle('Детально')
	...
	public function admin_display_new($params = array()){
		...
		$variables = array_merge($_POST, array(
			'instanceId' => 0,
			'pageTitle'  => $pageTitle,
			'validation' => Voms::Create()->getValidator()->getJsRules(),
	...
	public function action_delete($params = array()){
		...
		$this->setRedirectUrl('admin/content/voms/list');
